namespace Exceptions
{
    ...
    public class AssertDetector_Devourer
    {
        protected readonly static ILogger myLogger = LogTraceService.Instance.GetLogger("AssertDetectorLogger");

        private static AssertDetector_Devourer sAssertDetector = null;

        private long[] mLastPrivateErrorTime = new long[Constants.MARKET_COUNT];

        public static AssertDetector_Devourer getInstance()
        {
            if (sAssertDetector == null)
            {
                sAssertDetector = new AssertDetector_Devourer();
            }
            return sAssertDetector;
        }

        private AssertDetector_Devourer()
        {
        }
